...
- Code indexing is delegated to `codebase-memory-mcp`. `MEMORY.md` holds only durable human decisions.
- v4: retired the bespoke code-map layer entirely; `codebase-memory-mcp` is the sole indexing layer. Version 4.0.0; banner "lets build it Ricardo - we are using v4 now".
